...
This is a simple shell script written for checking a program's correctness by checking its outputs with some sample outputs. 
I wrote this for checking my programming assignments. I did not debug very throughoutly so use it with caution.

# Description & How to use
1. I hard coded some of the file names and directories in the script. Configure the directories and rename your testcases and programs under the following rule so that it works with my script:
...
3. Run the script with 3 parameters:
    ...
    - As an example, to run the script for the foobar.exe program above, if the folder is named as "testcases" the command would be "./test.sh foobar 2 testcases"
